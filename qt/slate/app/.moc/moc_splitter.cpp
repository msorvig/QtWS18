/****************************************************************************
** Meta object code from reading C++ file 'splitter.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.12.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../qt-webassembly-examples/slate/app/splitter.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'splitter.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.12.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_Splitter_t {
    QByteArrayData data[12];
    char stringdata0[123];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Splitter_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Splitter_t qt_meta_stringdata_Splitter = {
    {
QT_MOC_LITERAL(0, 0, 8), // "Splitter"
QT_MOC_LITERAL(1, 9, 14), // "enabledChanged"
QT_MOC_LITERAL(2, 24, 0), // ""
QT_MOC_LITERAL(3, 25, 15), // "positionChanged"
QT_MOC_LITERAL(4, 41, 12), // "widthChanged"
QT_MOC_LITERAL(5, 54, 14), // "pressedChanged"
QT_MOC_LITERAL(6, 69, 14), // "hoveredChanged"
QT_MOC_LITERAL(7, 84, 7), // "enabled"
QT_MOC_LITERAL(8, 92, 8), // "position"
QT_MOC_LITERAL(9, 101, 5), // "width"
QT_MOC_LITERAL(10, 107, 7), // "pressed"
QT_MOC_LITERAL(11, 115, 7) // "hovered"

    },
    "Splitter\0enabledChanged\0\0positionChanged\0"
    "widthChanged\0pressedChanged\0hoveredChanged\0"
    "enabled\0position\0width\0pressed\0hovered"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Splitter[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       5,   44, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       5,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   39,    2, 0x06 /* Public */,
       3,    0,   40,    2, 0x06 /* Public */,
       4,    0,   41,    2, 0x06 /* Public */,
       5,    0,   42,    2, 0x06 /* Public */,
       6,    0,   43,    2, 0x06 /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

 // properties: name, type, flags
       7, QMetaType::Bool, 0x00495103,
       8, QMetaType::QReal, 0x00495801,
       9, QMetaType::Int, 0x00495103,
      10, QMetaType::Bool, 0x00495903,
      11, QMetaType::Bool, 0x00c95903,

 // properties: notify_signal_id
       0,
       1,
       2,
       3,
       4,

 // properties: revision
       0,
       0,
       0,
       0,
       1,

       0        // eod
};

void Splitter::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Splitter *_t = static_cast<Splitter *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->enabledChanged(); break;
        case 1: _t->positionChanged(); break;
        case 2: _t->widthChanged(); break;
        case 3: _t->pressedChanged(); break;
        case 4: _t->hoveredChanged(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (Splitter::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&Splitter::enabledChanged)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (Splitter::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&Splitter::positionChanged)) {
                *result = 1;
                return;
            }
        }
        {
            using _t = void (Splitter::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&Splitter::widthChanged)) {
                *result = 2;
                return;
            }
        }
        {
            using _t = void (Splitter::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&Splitter::pressedChanged)) {
                *result = 3;
                return;
            }
        }
        {
            using _t = void (Splitter::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&Splitter::hoveredChanged)) {
                *result = 4;
                return;
            }
        }
    }
#ifndef QT_NO_PROPERTIES
    else if (_c == QMetaObject::ReadProperty) {
        Splitter *_t = static_cast<Splitter *>(_o);
        Q_UNUSED(_t)
        void *_v = _a[0];
        switch (_id) {
        case 0: *reinterpret_cast< bool*>(_v) = _t->isEnabled(); break;
        case 1: *reinterpret_cast< qreal*>(_v) = _t->position(); break;
        case 2: *reinterpret_cast< int*>(_v) = _t->width(); break;
        case 3: *reinterpret_cast< bool*>(_v) = _t->isPressed(); break;
        case 4: *reinterpret_cast< bool*>(_v) = _t->isHovered(); break;
        default: break;
        }
    } else if (_c == QMetaObject::WriteProperty) {
        Splitter *_t = static_cast<Splitter *>(_o);
        Q_UNUSED(_t)
        void *_v = _a[0];
        switch (_id) {
        case 0: _t->setEnabled(*reinterpret_cast< bool*>(_v)); break;
        case 2: _t->setWidth(*reinterpret_cast< int*>(_v)); break;
        case 3: _t->setPressed(*reinterpret_cast< bool*>(_v)); break;
        case 4: _t->setHovered(*reinterpret_cast< bool*>(_v)); break;
        default: break;
        }
    } else if (_c == QMetaObject::ResetProperty) {
    }
#endif // QT_NO_PROPERTIES
    Q_UNUSED(_a);
}

QT_INIT_METAOBJECT const QMetaObject Splitter::staticMetaObject = { {
    &QObject::staticMetaObject,
    qt_meta_stringdata_Splitter.data,
    qt_meta_data_Splitter,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *Splitter::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Splitter::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_Splitter.stringdata0))
        return static_cast<void*>(this);
    return QObject::qt_metacast(_clname);
}

int Splitter::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 5;
    }
#ifndef QT_NO_PROPERTIES
   else if (_c == QMetaObject::ReadProperty || _c == QMetaObject::WriteProperty
            || _c == QMetaObject::ResetProperty || _c == QMetaObject::RegisterPropertyMetaType) {
        qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::QueryPropertyDesignable) {
        _id -= 5;
    } else if (_c == QMetaObject::QueryPropertyScriptable) {
        _id -= 5;
    } else if (_c == QMetaObject::QueryPropertyStored) {
        _id -= 5;
    } else if (_c == QMetaObject::QueryPropertyEditable) {
        _id -= 5;
    } else if (_c == QMetaObject::QueryPropertyUser) {
        _id -= 5;
    }
#endif // QT_NO_PROPERTIES
    return _id;
}

// SIGNAL 0
void Splitter::enabledChanged()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}

// SIGNAL 1
void Splitter::positionChanged()
{
    QMetaObject::activate(this, &staticMetaObject, 1, nullptr);
}

// SIGNAL 2
void Splitter::widthChanged()
{
    QMetaObject::activate(this, &staticMetaObject, 2, nullptr);
}

// SIGNAL 3
void Splitter::pressedChanged()
{
    QMetaObject::activate(this, &staticMetaObject, 3, nullptr);
}

// SIGNAL 4
void Splitter::hoveredChanged()
{
    QMetaObject::activate(this, &staticMetaObject, 4, nullptr);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
